 ...
 * based on a configuration property (using the built-in {@linkplain io.quarkus.ts.openshift.common.config.Config configuration system}).
...
public @interface OnlyIfConfigured {
    /**
     * Configuration key that must be enabled (per {@link io.quarkus.ts.openshift.common.config.Config#getAsBoolean(String, boolean) Config.getAsBoolean})
