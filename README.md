...
#### Deployment using Quarkus docker build strategy

If the `@OpenShiftTest` annotation is configured to use the strategy `UsingQuarkusPluginDeploymentStrategy.UsingDockerStrategy`
and the module contains the `quarkus-openshift` extension, the test framework will use Quarkus to orchestrate a docker
build of the module and its deployment to OpenShift. Note that the property `quarkus.kubernetes.deploy` must NOT be set
in the module configuration in order to avoid the deployment at build time. Also note that the default dockerfiles
generated by Quarkus project bootstrapping must be present for this strategy to work.
